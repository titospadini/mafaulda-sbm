...
def extract_features_for_file(filepath: str, use_hann: bool = False, use_fixed_entropy: bool = False) -> np.ndarray:
    """
    Extracts exactly 46 diagnostic features from a single CSV time-series scenario file.

    Pedagogical Context:
        To perform rotating-machine fault diagnosis, raw vibration, acoustic, and rotation
        time-series must be compressed into representative statistical and spectral metrics.
        This function implements three layers of feature engineering:

        1. Rotation Frequency (fr) Estimation:
           - Computes the Discrete Fourier Transform (DFT) of the tachometer pulse train (channel 7).
           - Identifies the dominant frequency peak within the physically constrained range [5, 120] Hz.
             This range filters out high-frequency sensor noise and low-frequency DC baseline drift.

        2. Spectral Magnitude Features (21 features):
           - Extracts exact frequency magnitudes for the first 7 physical sensors at the fundamental
             frequency (fr), its second harmonic (2fr), and third harmonic (3fr).
           - Because these harmonic frequencies may not align exactly with discrete FFT bin centers,
             linear interpolation (np.interp) is used to calculate high-fidelity continuous spectral peaks.
           - Total features = 7 sensors * 3 harmonics = 21 features.

        3. Statistical Signal Features (24 features):
           - Computes three core descriptors across all 8 sensor channels (8 * 3 = 24 features):
             a. Mean: Represents physical baseline shifts or offsets.
             b. Shannon Entropy: Quantifies structural complexity and noise levels, calculated via
                probability bin counts of a 100-bin histogram.
             c. Kurtosis: Measures the "peakedness" of the signal distribution, identifying mechanical
                impact impulses (sudden shocks/clicks caused by gear tooth cracks or bearing spalls).

        4. DSP Enhancements (Optional Flags):
           - Hanning Window (`use_hann`): Dampens the start and end of the signal to zero to prevent
             spectral leakage. It applies coherent gain correction (scale factor 2.0) to maintain amplitude fidelity.
           - Fixed Entropy Range (`use_fixed_entropy`): Locks the histogram calculation range to `(-10.0, 10.0)`.
             This resolves a critical bug where dynamic histogram bin scaling causes a sudden mechanical shock impulse
    ...
    Parameters:
        ...
        use_hann (bool): Whether to apply a Hanning window and coherent gain correction to FFT calculations.
...
def process_set_parallel(filepaths: List[str], set_name: str, use_hann: bool = False, use_fixed_entropy: bool = False) -> np.ndarray:
    """
    Spawns concurrent worker processes to extract the 46 hand-crafted features in parallel across
    all CSV files in a given dataset split.

    Pedagogical Context:
        Processing 1,951 files sequentially would require significant computation time due to I/O overhead
        and CPU-bound Fast Fourier Transforms (FFT). By leveraging a ProcessPoolExecutor, this function
        distributes the workload across all available CPU cores, achieving optimal multi-core performance.
        It uses `functools.partial` to cleanly package the runtime DSP flags for parallel execution.

    Parameters:
        filepaths (List[str]): List of absolute file paths pointing to the raw CSV files.
        set_name (str): Human-readable name of the dataset split (e.g., 'Training' or 'Testing') for log outputs.
        use_hann (bool): Whether to apply a Hanning window and coherent gain correction to FFT.
        use_fixed_entropy (bool): Whether to use a fixed histogram range (-10.0, 10.0) for Shannon entropy.

    Returns:
        np.ndarray: A 2D feature matrix of shape (num_samples, 46) containing the extracted features.
    """
