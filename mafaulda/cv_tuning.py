...
def run_tuning(
    ...
    if has_test_set and X_test is not None and y_test is not None:
        ...
        X_train_ext = generate_extended_features(X_train, D_c_full, gamma=best_gamma)
        X_test_ext = generate_extended_features(X_test, D_c_full, gamma=best_gamma)
...
def run_nested_cv(
    ...
    for outer_fold, (outer_tr_idx, outer_val_idx) in enumerate(outer_cv.split(X_train, y_train)):
        ...
        for gamma in gammas:
            for tau in taus:
                ...
                for in_tr_idx, in_val_idx in inner_cv.split(X_out_tr, y_out_tr):
                    ...
                    X_in_tr_ext = generate_extended_features(X_in_tr, D_c_dict, gamma=gamma)
                    X_in_val_ext = generate_extended_features(X_in_val, D_c_dict, gamma=gamma)
        ...
        X_out_tr_ext = generate_extended_features(X_out_tr, D_c_dict, gamma=best_gamma)
        X_out_val_ext = generate_extended_features(X_out_val, D_c_dict, gamma=best_gamma)
